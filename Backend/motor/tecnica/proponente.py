...
_COMPOSICION_RE = re.compile(
    r"COMPOSICION\s+ACCIONARIA|CERTIFICACION\s+ACCIONARIA|CERTIFICA(?:DO|CION)\s+DE\s+(?:LOS\s+)?(?:SOCIOS|ACCIONISTAS)"
    r"|SOCIOS\s+Y(?:/?O)?\s+ACCIONISTAS?|CONFORMACION\s+(?:ACCIONARIA|DE\s+LA\s+(?:PERSONA|SOCIEDAD))"
    r"|LITERAL\s+E\s+DEL\s+ARTICULO\s+9\s+DEL\s+DECRETO\s+1510"
