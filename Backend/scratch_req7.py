"""Requisito 7 (objeto social) evaluado directo en los 4 procesos, frente al
abogado. Solo mide."""
...
from motor.evaluacion.camara_comercio import evaluar_requisito7
...
def una(args):
    ...
    r = evaluar_requisito7(pdfs, OBJETOS[proceso], obtener_tipo_proponente(pdfs))
    return proceso, p.hoja, r.cumple, r.motivo

if __name__ == "__main__":
    abogado = {f"{p}/{c['hoja']}": c["ground_truth"] for p, f in GT.items() for c in json.load(open(f)) if c["requisito"] == 7}
    ...
    with ProcessPoolExecutor(4) as ex:
        for proceso, hoja, cumple, motivo in ex.map(una, tareas):
            ...
            if not cumple and "no se relaciona" in (motivo or ""):
