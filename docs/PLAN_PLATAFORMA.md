...
| **F7. El pliego define la evaluación** ✅ | Lectura completa del pliego (todas las páginas, OCR donde haga falta) y división en secciones; detectores de lo que cambia la evaluación (vigencias en días, duración de la sociedad, identidad del representante, limitación a MiPyme, verificaciones no incluidas en la plantilla) con cita literal y página; decisión obligatoria de una persona por cada ajuste; verificación manual para lo que el motor no revisa; reporte y expediente explican cada ajuste; caché por huella del PDF y por entidad | Sobre el pliego real de 96 páginas: detecta los 3 ajustes y las aclaraciones en 13 s y reutiliza el análisis al volver a subirlo. |
